...
/**
 * Visaire Agent implementation with Forge-style sophisticated reasoning
...
class EnhancedAgent extends Agent {
  constructor(options = {}) {
    ...
    const enhancedOptions = {
      ...
      tools: {
        ...
        autoApprove: options.autoApprove !== false, // Default to true for immediate execution
        confirmationRequired: false, // No confirmation required for tool execution
  ...
  async processPrompt(prompt, options = {}) {
    try {
      ...
      this.logger.info('Processing prompt with visaire agent', {
      ...
      const parsedActions = this.parseActionsFromResponse(llmResponse);

      // Step 3: Process with enhanced reasoning (but override actions)
      const result = await this.processInput(prompt, {
        llmResponse,
        overrideActions: parsedActions,
        ...processOptions
      });
  ...
  parseActionsFromResponse(response) {
    ...
    for (const line of actionLines) {
      const actionText = line.substring(1).trim(); // Remove the '-' prefix
      
      if (actionText.startsWith('CREATE_FILE:') || actionText.startsWith('WRITE_FILE:')) {
        const match = actionText.match(/(CREATE_FILE|WRITE_FILE):\s*([^|]+)\s*\|\s*(.+)/);
        if (match) {
          ...
          actions.push({
            ...
            parameters: [filePath.trim(), content.trim()],
            description: `${actionType.toLowerCase().replace('_', ' ')} ${filePath.trim()}`
          });
        }
      } else if (actionText.startsWith('READ_FILE:')) {
        const match = actionText.match(/READ_FILE:\s*(.+)/);
        if (match) {
      ...
      } else if (actionText.startsWith('CREATE_FOLDER:')) {
        const match = actionText.match(/CREATE_FOLDER:\s*(.+)/);
        if (match) {
      ...
      } else if (actionText.startsWith('LIST_DIRECTORY:')) {
        const match = actionText.match(/LIST_DIRECTORY:\s*(.+)/);
        if (match) {
      ...
      } else if (actionText.startsWith('RUN_COMMAND:')) {
        const match = actionText.match(/RUN_COMMAND:\s*(.+)/);
        if (match) {
      ...
      } else if (actionText.startsWith('INSTALL_PACKAGE:')) {
        const match = actionText.match(/INSTALL_PACKAGE:\s*(.+)/);
        if (match) {
          ...
          actions.push({
            ...
            parameters: [`npm install ${packageName}`],
...
Please analyze this request carefully and provide a comprehensive response. If the request requires actions to be taken (like creating files, running commands, or making changes), format your response as follows:

1. First, provide your analysis and reasoning
2. Then, if actions are needed, include an ACTIONS section with specific commands:

ACTIONS:
[List specific actions using these formats:]
...
ACTIONS:
- CREATE_FOLDER: test
- CREATE_FILE: test/index.html | <html><body>Hello World</body></html>
- RUN_COMMAND: ls -la

If no actions are needed, just provide your response without an ACTIONS section.`;
  ...
  async startAutonomousSession(prompt, options = {}) {
    ...
    try {
      ...
      const sessionOptions = {
        maxIterations: options.maxIterations || 10,
        autoApprove: true, // Autonomous mode always auto-approves tool and command actions
