...
class InteractiveSession {
   ...
   */
  setupReadline() {
    // Check if input is from a TTY (interactive terminal) or redirected (file/pipe)
    ...
    // For non-interactive input (file/pipe), read all lines upfront
  ...
  async conversationLoop() {
    while (this.isRunning) {
      try {
        ...
        if (error.message === 'NO_MORE_INPUT') {
          // For non-interactive mode, gracefully exit when all input is processed
          Utils.logInfo('All input processed successfully');
          await this.handleExit();
          break;
        }
  ...
  getUserInput() {
    return new Promise((resolve, reject) => {
      if (this.isInteractive) {
        // Interactive mode: use traditional readline question
        ...
        // Non-interactive mode: process pre-loaded lines
        if (!this.inputReady) {
          // Wait a bit for input to be fully loaded
          setTimeout(() => {
            this.getUserInput().then(resolve).catch(reject);
          }, 10);
          return;
        }
        
        if (this.currentLineIndex >= this.inputLines.length) {
          // No more lines to process
          reject(new Error('NO_MORE_INPUT'));
          return;
        }
        
        const line = this.inputLines[this.currentLineIndex++];
        console.log(chalk.cyan('> ') + line); // Display the prompt and input for clarity
        resolve(line);
      }
    });
  }
