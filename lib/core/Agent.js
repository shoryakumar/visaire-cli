...
class Agent extends EventEmitter {
  constructor(options = {}) {
    ...
    const schema = Joi.object({
      ...
      tools: Joi.object({
        ...
        autoApprove: Joi.boolean().default(true), // Default to true for immediate tool execution
  ...
  async processInput(input, options = {}) {
    ...
    try {
      ...
      const actionsToExecute = options.overrideActions || reasoning.actions;
      if (actionsToExecute && actionsToExecute.length > 0) {
        this.state = 'acting';
        executionResults = await this.executeActions(actionsToExecute, {
          conversationId,
